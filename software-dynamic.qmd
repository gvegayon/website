  ...
  tryCatch({
    ...
    if (status_code(response) == 200) {
      ...
      parse_bibtex_entry <- function(entry) {
        ...
        title <- if (!is.na(title_match[1,2])) title_match[1,2] else "Untitled"
        
        # Extract URL
        url_match <- str_match(entry, "url\\s*=\\s*\\{([^}]+)\\}")
        url <- if (!is.na(url_match[1,2])) url_match[1,2] else ""
        
        # Extract note/description
        note_match <- str_match(entry, "note\\s*=\\s*\\{([^}]+)\\}")
        note <- if (!is.na(note_match[1,2])) note_match[1,2] else ""
      ...
      entries <- entries[entries != "" & nchar(trimws(entries)) > 0] # Remove empty entries
      ...
      for (i in 2:length(entries)) { # Start from 2 since first element is usually empty
        ...
        if (parsed$title != "Untitled" && parsed$title != "") {
          if (parsed$url != "") {
